...
function getIconType(step) {
    ...
    if (travelMode === "TRANSIT") {
        ...
        vehicleType = step.getElementsByTagName("transit_details")[0]
...
function getStepInfo(step) {
    ...
    var s = "",
        ...
        htmlInstructions = step.getElementsByTagName("html_instructions")[0]
        .childNodes[0].nodeValue,
        travelMode = step.getElementsByTagName("travel_mode")[0]
        .childNodes[0].nodeValue,
        transitShortName;
...
/**
 * Displays directions in the result card.
 * @param {xmlDoc} xmlDoc - xml document that contains the result of the API query.
 */
function displayDirections(xmlDoc) {
    ...
    var resultArea = document.getElementById("results"),
        ...
        totalDistance = leg.getElementsByTagName("distance")
        [leg.getElementsByTagName("distance").length - 1].querySelector("text").childNodes[0].nodeValue,
        totalDuration = leg.getElementsByTagName("duration")
        [leg.getElementsByTagName("duration").length - 1].querySelector("text").childNodes[0].nodeValue,
...
/**
 * Displays an alert containing an error message.
...
/**
 * Loads and processes data from XMLHttpRequest response.
 * @param {string} response - The response after a successful XMLHttpRequest.
 */
...
/**
 * Does form validation to ensure various fields are not empty.
...
/**
 * Main function called from the form to being an API request to Google's
 * Map Direction API using the parameters obtained from the form's input fields.
 */
